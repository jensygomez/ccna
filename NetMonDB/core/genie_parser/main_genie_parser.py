...
def genie_main(raw_output):
    ...
    Devuelve datos mínimos en dict listo para la tabla.
    """
    if not raw_output:
        print("⚠️ No hay output para parsear.")
        return None

    print("🧩 Parseando datos con Genie (mínimo)...")
    parsed_data = parse_show_with_genie(raw_output)
    print(f"✅ JSON guardado en outputs/{parsed_data['hostname']}_running_config.json")
    return parsed_data
